...
def process(data, keyword):
    ...
    def modify_info(info):
        return ['+0' + item if item.count('+') + item.count('-') == 1 else item for item in info] or ['+0+0']
    ...
    processed_data = [calculate_totals(modify_info(extract_info(entry))) for entry in data]
    print("提取数据：")
